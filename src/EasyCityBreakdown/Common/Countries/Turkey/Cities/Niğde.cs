...
namespace EasyCityBreakdown.Common.Cities.Turkey
{
    public class Niğde : AbstractCity
    {
        ...
        private List<Breakdown> GetElectricBreakdowns()
        {
            ...
            var districtsReq = new RestRequest("https://cc.meramedas.com.tr/services/publicdata.ashx?m=mrm_gb1_ilce&il=51");
            var districts = JsonConvert.DeserializeObject<dynamic>(client.Get<dynamic>(districtsReq).ToString());

            foreach (var district in districts)
            {
                var breakdownRequest = new RestRequest($"https://cc.meramedas.com.tr/services/publicdata.ashx?m=mrm_gb1&il=51&ilce={(string)district.Name}&mahalle=&ay=&yil=");
